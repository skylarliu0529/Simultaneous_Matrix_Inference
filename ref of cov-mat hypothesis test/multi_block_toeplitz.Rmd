...
```
### **Toeplitz Matrix**
A **Toeplitz matrix** is a matrix where each descending diagonal from left to right is constant. For example:
...
For example, with 3 blocks of size 4, the covariance matrix might look like this:
...
Where each block \( \Sigma^{(i)} \) is a Toeplitz matrix. For instance, \( \Sigma^{(1)} \) could be:
...
\]
```{r}
sigma_0 <- function(p, block_size = 4, rho = 0.8) {
  # Determine the number of full blocks
  num_blocks <- floor(p / block_size)
  Sigma <- matrix(0, nrow = p, ncol = p)
  # Assign each row/column a block membership (0 for leftover rows/columns)
  block_membership <- c(
    rep(seq_len(num_blocks), each = block_size),
    rep(0, p - num_blocks * block_size)
  )
  # Create a logical matrix indicating which entries belong to the same nonzero block
  same_block_matrix <- outer(block_membership, block_membership, 
                             FUN = function(x, y) (x == y) & (x != 0))
  
  diff_mat <- abs(row(Sigma) - col(Sigma))
  toeplitz_mat <- rho^diff_mat
  Sigma[same_block_matrix] <- toeplitz_mat[same_block_matrix]
